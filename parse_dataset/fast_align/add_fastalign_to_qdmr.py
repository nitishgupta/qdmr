...
def add_fastalign_to_qdmr(qdmr_examples: List[QDMRExample],
                          progtype: str, inorder: bool, qids: List[str],
...
def get_fastalign_format_filename(progtype, inorder):
    inorderstr = ""
    if progtype == "seq2seq" and inorder:
        inorderstr = ".inorder"
    # Example -- train.fast_align.seq2seq.inorder
    fastalign_outfile = "all_examples.fast_align" + f".{progtype}" + inorderstr
    qids_outfile = "all_examples.fast_align" + ".qids"

    return fastalign_outfile, qids_outfile



def main(args):

    train_qdmr_examples: List[QDMRExample] = read_qdmr_json_to_examples(os.path.join(args.input_dir, "train.json"))
    dev_qdmr_examples: List[QDMRExample] = read_qdmr_json_to_examples(os.path.join(args.input_dir, "dev.json"))
    ...
    for progtype in ["seq2seq", "grammar"]:
        for inorder in [True, False]:
            ...
            # Example -- train.fast_align.seq2seq.inorder
            fastalign_filename, qids_filename = get_fastalign_format_filename(progtype, inorder)

            fastalign_filename = fastalign_filename + ".output"

            print("Processing: {}".format(fastalign_filename))

            fast_align_output_path = os.path.join(args.input_dir, fastalign_filename)
            fast_align_outputs = read_list_string_file(fast_align_output_path)

            qids_output_path = os.path.join(args.input_dir, qids_filename)
            qids = read_list_string_file(qids_output_path)

            updated_train_qdmr_examples = add_fastalign_to_qdmr(qdmr_examples=train_qdmr_examples,
                                                                progtype=progtype,
                                                                inorder=inorder, qids=qids,
                                                                fast_align_outputs=fast_align_outputs)

            updated_dev_qdmr_examples = add_fastalign_to_qdmr(qdmr_examples=dev_qdmr_examples,
                                                              progtype=progtype,
                                                              inorder=inorder, qids=qids,
                                                              fast_align_outputs=fast_align_outputs)

            print("Written outputs back to : {}".format(args.input_dir))
            utils.write_qdmr_examples_to_json(updated_train_qdmr_examples, os.path.join(args.input_dir, "train.json"))
            utils.write_qdmr_examples_to_json(updated_dev_qdmr_examples, os.path.join(args.input_dir, "dev.json"))
